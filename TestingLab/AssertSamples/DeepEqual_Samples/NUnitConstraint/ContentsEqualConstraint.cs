...
namespace DeepEqual_Samples.NUnitConstraint
{
    /*
     * 
     * found http://stackoverflow.com/questions/2046121/how-to-compare-two-objects-in-unit-test
     * "plugin" of nunit can be used for deep comparison
     * 
     * */

    public static class MyIs
    {
        public static MyConstraint My(object expected)
        {
            return new MyConstraint(expected);
        }            
    }

    [TestFixture]
    public class NUnitConstraintSamples
    {
        [Test]
        public void Strings_MyConstraint()
        {
            const string expected = "Joe";
            const string actual = "joeu";

            Assert.That(actual, MyIs.My(expected));
        }

        [Test]
        public void Strings_EqualConstraint()
        {
            const string expected = "Joe";
        ...
        [Test]
        public void Strings_ContentsEqualConstraint()
        {
            const string expected = "Joe";
            const string actual = "Joe";

            var contentsEqualConstraint = new ContentsEqualConstraint(expected);
            Assert.That(actual, Has.Some.Matches(contentsEqualConstraint));
        }
        [Test]
        public void Ints_ContentsEqualConstraint()
        {
    ...
    public class MyConstraint : Constraint
    {
        ...
        {
            writer.DisplayDifferences(m_expected, m_actual);
        }
        #endregion
    }
    ...
    public class ContentsEqualConstraint : Constraint
    {
        ...
        private readonly IDictionary<Type, Func<object, object, bool>> _predicates =
        ...
        private void MatchesFields(object expectedValue, object actualValue, Constraint equalConstraint,
            ref bool matches)
        {
            Type expectedType = expectedValue.GetType();
        ...
        {
            get { return typePath.Count < _maxRecursion; }
        }
